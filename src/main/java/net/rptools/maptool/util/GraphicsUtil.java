...
public class GraphicsUtil {
    ...
    public static String getColourHexString(Color colour){
        if(colour == null){
            return "#000";
        }
    ...
    public static void renderSoftClipping(Graphics2D g, Shape shape, int width) {
    ...
    public static Shape convexHull(Shape shape) {
        PathIterator pi = shape.getPathIterator(null);
        Geometry convexHull = ShapeReader.read(pi, GeometryUtil.getGeometryFactory()).convexHull();
        return new ShapeWriter().toShape(convexHull);
    }

    /**
     *
     * @param shape AWT Shape to modify
     * @param roundness 0 is linear, 1 is round, >1 is increasingly curved
     * @return A shape with rounded corners
     */
    public static Shape roundedShape(Shape shape, double roundness){
        PathIterator pi = shape.getPathIterator(null);
        Geometry geometry = ShapeReader.read(pi, GeometryUtil.getGeometryFactory());
        geometry = CubicBezierCurve.bezierCurve(geometry, roundness);
        return new ShapeWriter().toShape(geometry);
    }
    public static Shape roundedShape(Shape shape, double roundness, int recursions){
        for (int i = 0; i < recursions ; i++) {
            shape = roundedShape(shape, roundness);
        }
        return shape;
    }
    public static Shape snowflake(){
        KochSnowflakeBuilder snowflakeBuilder = new KochSnowflakeBuilder(GeometryUtil.getGeometryFactory());
        snowflakeBuilder.setNumPoints(6);
        snowflakeBuilder.setExtent(new Envelope(0,0,200,200));
        return new ShapeWriter().toShape(snowflakeBuilder.getGeometry());
    }
