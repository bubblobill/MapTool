...
public class ImageUtil {
    ...
    public static Color brightenDarken(Color color, float... factor) {
        ...
        int b = (rgb & 0xFF);
        int mean = (r + g + b) / 3;
        r = (int) Math.clamp(r + mean * amount, 0, 255);
        g = (int) Math.clamp(g + mean * amount, 0, 255);
        b = (int) Math.clamp(b + mean * amount, 0, 255);
        return new Color(r, g, b);
    }

    public AffineTransform centreImageTransform(BufferedImage bi) {
        return AffineTransform.getTranslateInstance(bi.getWidth() / 2d, bi.getHeight() / 2d);
    }
}
